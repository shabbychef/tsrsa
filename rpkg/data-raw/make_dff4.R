...
ff_read <- function(stub,
	...
	if (monthly) {
		...
		TEO <- as.Date(data$Date,format="%Y%m%d") 
	}
	# to xts:
...
FF4 <- ff_read('F-F_Research_Data_Factors_daily_TXT',
							 subfile='F-F_Research_Data_Factors_daily.txt',
							 monthly=FALSE,
							 skip=4,header=FALSE,widths=c(8,rep(8,4)))
FFM <- ff_read('F-F_Momentum_Factor_daily_TXT',
							 subfile='F-F_Momentum_Factor_daily.txt',
							 monthly=FALSE,
							 skip=13,header=FALSE,widths=c(8,rep(8,1)))
colnames(FFM) <- c("UMD")
...
# subselect. 
dff4 <- ff4.xts['1900-01-01::2025-11-30',]
